...
def main():
    ...
    num = 0
    uttid_to_feats = {}
    with open(src_scp) as f:
        for line in f:
            uttid, feats = line.rstrip().split()
            uttid_to_feats[uttid] = feats

    num = 0
    with open(tgt_scp) as f1:
        for line in f1:
            uttid, tgt_feats = line.rstrip().split()
            if uttid in list(uttid_to_feats.keys()):
                num += 1
                src_feats = uttid_to_feats[uttid]
                embeddings_a = torch.tensor(kaldiio.load_mat(src_feats))
                embeddings_b = torch.tensor(kaldiio.load_mat(tgt_feats)[1])
