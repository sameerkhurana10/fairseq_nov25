...
@register_criterion("st_crit_a", dataclass=STCriterionConfig)
class STCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if net_output["padding_mask"] is not None:
            ...
            input_lengths = enc_out.new_full(
                (enc_out.size(1),), enc_out.size(0), dtype=torch.long
            )
        audio_embed = self.avg_pool(enc_out.permute(1, 2, 0).float(), input_lengths)
        # L2 normalize
        audio_embed = F.normalize(audio_embed, p=2)
        # for sentences text embeddings are already normalized but for word embedding its not
        text_embed = sample["text_embedding"]
        text_embed = F.normalize(text_embed, p=2)
        loss = F.l1_loss(audio_embed.float(), text_embed.float(), reduction="sum")
