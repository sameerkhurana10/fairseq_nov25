...
class Wav2VecFeatureReader(object):
    ...
    def get_feats(self, loc):
        ...
        with torch.no_grad():
            ...
            # print(audio_embed.squeeze().tolist())
            # exit(1)
            audio_embed = F.normalize(audio_embed, p=2).squeeze()
            print(audio_embed.tolist())
            exit(1)
            return audio_embed
    ...
    with open(tsv, "r") as fp:
        ...
        def iterate():
            for i, fname in enumerate(wavs):
                ...
                yield uttids[i], w2v_feats
    return iterate, num

generator, num = get_iterator(wav_scp, nshard, rank)
iterator = generator()

os.makedirs(feat_dir, exist_ok=True)

feat_path = f"{feat_dir}/xlsr_{rank}_{nshard}.scp"
