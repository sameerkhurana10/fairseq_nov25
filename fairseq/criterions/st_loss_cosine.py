...
@register_criterion("st_crit_cos", dataclass=STCriterionConfig)
class STCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if text_embed.shape[1] == 2:
            ...
            raise Exception
        B, S = audio_embed.shape
        loss = 0.
        for b in range(B):
            cosine_sim = torch.dot(audio_embed[b].float(), text_embed[b].float())
            loss += 1.0 - cosine_sim

        # scale the loss to avoid vanishing gradient
        loss *= 100.0
