...
def get_parser():
    ...
    parser.add_argument('--skip-first', type=str, default='no')
    # fmt: on
...
class Wav2VecFeatureReader(object):
    ...
    def get_feats(self, loc, labse_loc):
        ...
        with torch.no_grad():
            ...
            if text_embed.shape[0] == 2:
                text_embed = torch.from_numpy(text_embed).float()[1]
            else:
                text_embed = torch.from_numpy(text_embed).float()[0]
            ...
            return audio_embed.cpu().squeeze(), text_embed
...
def get_iterator(args):
    with open(osp.join(args.data, args.split) + ".tsv", "r") as fp:
        lines = fp.read().split("\n")
        root = None
        if args.skip_first.lower() == 'yes':
            root = lines.pop(0).strip()
        files = [line.split("\t")[0] for line in lines if len(line) > 0]
