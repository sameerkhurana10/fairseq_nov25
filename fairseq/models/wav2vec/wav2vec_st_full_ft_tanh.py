...
@register_model("wav2vec_st_full_ft_tanh", dataclass=Wav2Vec2STConfig)
...
class Wav2VecEncoder(FairseqEncoder):
    ...
    def forward(self, source, padding_mask=None, **kwargs):
        ...
        if padding_mask is not None:
            ...
            input_lengths = x.new_full(
                (x.size(1),), x.size(0), dtype=torch.long
            )

        dtype = x.dtype
        x = self.avg_pool(x.permute(1, 2, 0).float(), input_lengths)

        if self.proj:
            x = self.proj(x.to(dtype=dtype))
