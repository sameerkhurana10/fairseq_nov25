...
@register_model("wav2vec_st_baseline", dataclass=Wav2Vec2STConfig)
...
class Wav2VecEncoder(FairseqEncoder):
    ...
    def forward(self, source, padding_mask=None, **kwargs):
        ...
        # don't fine-tune any of the model parameters.
        with torch.no_grad():
            res = self.w2v_model.extract_features(**w2v_args)
            layer_results = res["layer_results"]
            padding_mask = res["padding_mask"]
            # we take an average of layer results
            # taking equal contribution from each layer
            x = 0.
            for l in layer_results:
                x = x + l[0]
            x = x * 1. / len(layer_results)
