...
class JointSpeechLaBSEDataset(RawAudioDataset):
    def __init__(
        self,
        manifest_path,
        sample_rate,
        ...
        with open(manifest_path, "r") as f:
            ...
            for i, line in enumerate(f):
                ...
                if min_sample_size is not None and sz < min_sample_size:
                    ...
                    continue
                self.fnames.append(self.text_compressor.compress(items[0]))
                self.fnames_labse.append(self.text_compressor.compress(items[-1]))
                sizes.append(sz)
        logger.info(f"loaded {len(self.fnames)}, skipped {skipped} samples")
    ...
    def __getitem__(self, index):
        ...
        path_or_fp = os.path.join(self.root_dir, fn)
        wav, curr_sample_rate = sf.read(path_or_fp, dtype="float32")
        fn_labse = self.fnames_labse[index]
        fn_labse = self.text_compressor.decompress(fn_labse)
        text_embedding = kaldiio.load_mat(fn_labse).copy()

        feats = torch.from_numpy(wav).float()
        feats = self.postprocess(feats, curr_sample_rate)
        feats_labse = torch.from_numpy(text_embedding).float()
        return {"id": index, "source": feats, "feats_labse": feats_labse}

    def collater(self, samples):
        samples = [s for s in samples if s["source"] is not None]
        if len(samples) == 0:
            return {}
        collated_sources, padding_mask = self.get_collated(samples, "source", pad=True)
        collated_feats_labse, _ = self.get_collated(samples, "feats_labse", pad=False)
        input = {"source": collated_sources}
        out = {"id": torch.LongTensor([s["id"] for s in samples])}
        input["padding_mask"] = padding_mask
        out["net_input"] = input
    ...
    def get_collated(self, samples, key, pad):
        ...
        if len(sources[0].shape) == 2:
            ...
            collated_sources = sources[0].new_zeros(len(sources), target_size)
        padding_mask = (
            torch.BoolTensor(collated_sources.shape).fill_(False) if pad else None
        )
        for i, (source, size) in enumerate(zip(sources, sizes)):
            diff = size - target_size
            if diff == 0:
...
# print("hi")
# ds = JointSpeechLaBSEDataset("/gpfsscratch/rech/iqh/upp27cx/covost/manifests/fr_en/dev.tsv", 16000)
# # print(ds)
# print(len(ds))
# # print(ds[1]["feats_labse"].shape)
# samples = []
...
# net_in = out["net_input"]
# print(net_in["source"].shape)
# print(net_in["feats_labse"].shape)
# print(net_in["padding_mask"].shape)
# print(net_in["feats_labse"][0][1])
